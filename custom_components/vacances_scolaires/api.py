...
from .const import ZONE_TIMEZONES, ZONES, ZONES_ACADEMIES, ZONES_DOMTOM

# Removed: from aiohttp import Fingerprint, SSLContext
...
class VacancesScolairesAPI:
    ...
    def __init__(
        ...
        _LOGGER.info(
            "Initialized API for Zone %s, Academy: %s, Timezone: %s, verify_ssl: %s",
            zone,
            self.academy,
            self.timezone_str,
            verify_ssl,
    ...
    def _load_from_cache_sync(self) -> bool:
        ...
        try:
            ...
            _LOGGER.info(
                "Loaded %d vacation periods from cache for Zone %s, Academy %s",
                len(self._vacances),
                self.zone,
                self.academy,
            )
    ...
    async def async_fetch_vacances(self) -> bool:
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                ...
                end_date_str = f"{next_year}-12-31"
                ...
                where_clauses.append(
                    f'start_date <= "{end_date_str}" AND end_date >= "{start_date_str}"'
                )
                ...
                _LOGGER.debug(
                    "Fetching from API for years %s-%s: %s with params: %s (verify_ssl=%s)",
                    current_year,
                    next_year,
                    VACANCES_API_URL,
                    params,
                ...
                async with session.get(
                    ...
                    if resp.status == 200:
                        ...
                        _LOGGER.info(
                            "Fetched %d vacation periods from API for Zone %s, Academy %s",
                            len(self._vacances),
                            self.zone,
                            self.academy,
                        )
            ...
            _LOGGER.error(
                "Failed to parse API response as JSON: %s", err, exc_info=True
            )
            if await self._load_from_cache():
    ...
    def _load_static_data(self) -> None:
        """Load static vacation data as a fallback."""
        _LOGGER.info("Loading static vacation data.")
        # For now, just clear the list or populate with dummy data
        self._vacances = []
        self._use_static_data = True
    ...
    def _parse_api_data(self, data: dict) -> None:
        ...
        if not isinstance(data, dict):
            _LOGGER.warning(
                "API response is not a dictionary, type: %s", type(data).__name__
            )
            self._load_static_data()
            return
        ...
        if "results" not in data:
            _LOGGER.warning(
                "No 'results' key in API response: %s",
        ...
        _LOGGER.debug(
            "Processing %d records from API for zone %s", len(results), self.zone
        )
        ...
        for record in results:
            try:
                ...
                if population_str and population_str not in ["-", "Élèves"]:
                    _LOGGER.debug(
                        "Skipping vacation '%s' with population '%s' (only keeping '-' or 'Élèves')",
                ...
                if self.academy and location_str and location_str != self.academy:
                    _LOGGER.debug(
                        "Skipping vacation '%s' with location '%s' (looking for '%s')",
                        record.get("description", "N/A"),
                        location_str,
                        self.academy,
                    )
                ...
                _LOGGER.debug(
                    "Added vacation: %s (%s to %s) for zone %s, academy %s",
                    vacance["name"],
                    vacance["start"],
                    vacance["end"],
                    zones_str,
                ...
                _LOGGER.debug(
                    "Failed to parse vacation record: %s, error: %s",
                    record.get("description", "N/A"),
                    e,
                )
                continue
        ...
        if not self._vacances:
            _LOGGER.warning(
                "No vacations found for Zone %s (%s), Academy %s in API response, loading static data",
                self.zone,
                zone_letter,
                self.academy,
            )
            ...
            _LOGGER.info(
                "Successfully parsed %d vacations for Zone %s, Academy %s",
                len(self._vacances),
                self.zone,
                self.academy,
            )
    ...
    def get_jours_avant_vacances(self) -> Optional[int]:
        ...
        if prochaines:
            today = datetime.now(self.timezone).date()
            prochaines_dict = prochaines  # Explicit cast
            start_date = prochaines_dict["start"]
    ...
    def get_jours_restants_vacances(self) -> Optional[int]:
        ...
        if vacances_en_cours:
            today = datetime.now(self.timezone).date()
            vacances_en_cours_dict = vacances_en_cours  # Explicit cast
            end_date = vacances_en_cours_dict["end"]
            if isinstance(end_date, datetime):
                end_date = end_date.date()
            delta = end_date - today
            return max(0, int(delta.days) + 1)  # +1 pour inclure le dernier jour
        return None

    def get_all_vacances_for_calendar(self) -> list[dict[str, Any]]:
